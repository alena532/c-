...
public:
   ...
   void find_previous(T1 key)
    ...
    Iterator end()
    {
        return Iterator(_find_maximum(root) ,this);
    }
 
private:
    Iterator _begin(Node<T1,T2> *leaf);
    Node<T1,T2>* _add_node(pair<T1,T2> &number,Node<T1,T2> *leaf);
    virtual void _preorder_print(Node<T1,T2> *leaf);
    virtual void _order_print(Node<T1,T2> *leaf);
...
public:
    Iterator(Node<T1,T2>* temp,Map<T1,T2> *MYSET): it(temp), myset(MYSET) {};
    Iterator& operator ++()
    {
